...
class PasteleriaChatbotApp:
    ...
    def inicializar_sistema(self):
        """Inicializa todos los componentes del sistema cargando TODOS los datos de /data"""
        try:
            # Cargar TODOS los datos de la pastelería
            productos = self.data_loader.cargar_productos()
            politicas = self.data_loader.cargar_politicas()
            faqs = self.data_loader.cargar_faqs()
            ejemplos_pedidos = self.data_loader.cargar_ejemplos_pedidos()
            clientes = self.data_loader.cargar_clientes_ejemplo()
            ingredientes = self.data_loader.cargar_ingredientes()
            eventos = self.data_loader.cargar_eventos_especiales()
            
            # Combinar TODOS los documentos
            todos_documentos = (
                productos + politicas + faqs + ejemplos_pedidos + 
                clientes + ingredientes + eventos
            )
            
            # Inicializar motor RAG con TODOS los documentos
            self.rag_engine.cargar_documentos(todos_documentos)
            
            st.success(f"✅ Sistema inicializado con {len(todos_documentos)} documentos de datos")
            return True
